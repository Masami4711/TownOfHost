            ...
            foreach (var pva in __instance.playerStates)
            {
                ...
                switch (seer.GetCustomRole())
                {
                    ...
                    case CustomRoles.MadScientist:
                        if (Options.MadScientistNoticeImpostor.GetBool() && seer.GetPlayerTaskState().IsTaskFinished) //seerがタスクを終えている
                            LocalPlayerKnowsImpostor = true;
                        if (Options.MadScientistCanSeeDeathReason.GetBool() && target.Data.IsDead) //変更対象が死人
                            pva.NameText.text += $"({Helpers.ColorString(Utils.GetRoleColor(CustomRoles.Doctor), Utils.GetVitalText(target.PlayerId))})";
                        break;
                }
