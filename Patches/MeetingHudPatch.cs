            ...
            foreach (var pva in __instance.playerStates)
            {
                ...
                switch (seer.GetCustomRole().GetRoleType())
                {
                    case RoleType.Impostor:
                        ...
                        if (target.Is(CustomRoles.Madmate) && target.GetPlayerTaskState().IsTaskFinished && Options.MadmateCanAlsoBeExposedToImpostor.GetBool())
                            pva.NameText.text += Helpers.ColorString(Utils.GetRoleColor(CustomRoles.Madmate), "★"); //変更対象にSnitchマークをつける
