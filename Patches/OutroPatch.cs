    ...
    class EndGamePatch
    {
        ...
        public static void Postfix(AmongUsClient __instance, [HarmonyArgument(0)] ref EndGameResult endGameResult)
        {
            ...
            foreach (var kvp in Main.PlayerStates.OrderBy(x => x.Value.RealKiller.Item1.Ticks))
            {
                ...
                KillLog += $"\n{date.ToString("T")} {Main.AllPlayerNames[targetId]}({Utils.GetSelfRoleName(targetId)}{Utils.GetSubRolesText(targetId)}) [{Utils.GetVitalText(kvp.Key)}]";
                if (killerId != byte.MaxValue && killerId != targetId)
                    KillLog += $"\n\t\t⇐ {Main.AllPlayerNames[killerId]}({Utils.GetSelfRoleName(killerId)}{Utils.GetSubRolesText(killerId)})";
            }
