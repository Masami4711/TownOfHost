            ...
            if (RoleText != null && __instance != null)
            {
                ...
                if (GameStates.IsInGame)
                {
                    ...
                    else if (seer.GetCustomRole().IsImpostor()) //seerがインポスター
                    {
                        ...
                        else if (target.Is(CustomRoles.Madmate) && target.GetPlayerTaskState().IsTaskFinished && Options.MadmateCanAlsoBeExposedToImpostor.GetBool()) //targetがタスクを終わらせたマッドスニッチ
                            Mark += Helpers.ColorString(Utils.GetRoleColor(CustomRoles.Madmate), "★"); //targetにマーク付与
                    }
